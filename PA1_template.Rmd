...
Average steps per day: `r sprintf("%.1f", mean(steps$steps))`. Median: `r median(steps$steps)`.
...
The maximum number of steps, on average, is taken in interval `r maxSteps$interval`
 (`r maxSteps$steps`)
...
```
```{r echo=FALSE}
stepsMeanNoNa <- mean(steps$steps)
stepsMedianNoNa <- median(steps$steps)
changeMean <- stepsMeanNoNa-stepsMean
changeMedian <- stepsMedianNoNa-stepsMedian
```

After imputing missing values the average steps per day is `r sprintf("%.1f", stepsMeanNoNa)` 
(`r sprintf("%+.2f", changeMean)` change), median is `r sprintf("%.1f", stepsMedianNoNa)` 
(`r sprintf("%+.2f", changeMedian)` change).
...
activity$typeOfDay <- factor(isWeekend, levels=c(F, T), labels=c('Weekday', 'Weekend'))
stepsByTypeOfDay <- aggregate(steps ~ interval + typeOfDay, activity, FUN=mean)

ggplot(stepsByTypeOfDay, aes(x=interval, y=steps)) + facet_grid(typeOfDay ~ .) + geom_line() + xlab('time interval')
```
